...
export default function App() {
  ...
  function savePositioning() {
    ...
    const { pageIdxGlobal, slotIdx, offsetX, offsetY } = positioning;
    setBinders((prev) => {
      const next = prev.map((b) => ({ ...b, pages: b.pages.map((p) => ({ ...p, slots: [...p.slots] })) }));
      const slot = next[binderIndex].pages[pageIdxGlobal].slots[slotIdx];
      if (slot) next[binderIndex].pages[pageIdxGlobal].slots[slotIdx] = { ...slot, offsetX, offsetY };
      return next;
    });
    setPositioning(null);
  }
...
function PositionEditor({ positioning, onMove, onSave, onCancel }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 45, padding: 24 }}>
      <p style={{ color: COLORS.text, fontSize: 13, marginBottom: 14, textAlign: "center" }}>
        Bild ziehen, um den Ausschnitt festzulegen
      </p>
      <div
        ref={boxRef}
