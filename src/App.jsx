...
// Single source of truth for how a (possibly grouped/tiled) custom image is
// positioned - used identically by the real grid AND the position editor
// preview, so the two can never drift out of sync with each other again.
function tileImageTransform(scale, offsetX, offsetY, tileRow, tileCol, tileRows, tileCols) {
  const panX = (offsetX ?? 50) - 50;
  const panY = (offsetY ?? 50) - 50;
  const tileX = -(tileCol / tileCols) * 100;
  const tileY = -(tileRow / tileRows) * 100;
  // Order matters: the tile-selection offset must stay a FIXED amount regardless
  // of zoom (applied last/outermost), otherwise tiles converge toward the same
  // spot as scale shrinks - which is exactly the "each box zooms on its own"
  // bug. Pan is applied before scale on purpose, so panning sensitivity still
  // scales naturally with zoom.
  return `translate(${tileX}%, ${tileY}%) scale(${scale ?? 1}) translate(${panX}%, ${panY}%)`;
}
                ...
                <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
                  <img
                    ...
                    style={{
                      ...
                      left: 0,
                      width: `${tileCols * 100}%`,
                      aspectRatio: `${tileCols * 2.5} / ${tileRows * 3.5}`,
                      objectFit: slot.type === "custom" ? "contain" : "cover",
                      transform: tileImageTransform(slot.scale, slot.offsetX, slot.offsetY, tileRow, tileCol, tileRows, tileCols),
                ...
                {isMember && (
                  <img
                    ...
                    style={{
                      ...
                      top: 0,
                      left: 0,
                      width: `${positioning.tileCols * 100}%`,
                      aspectRatio: `${positioning.tileCols * 2.5} / ${positioning.tileRows * 3.5}`,
                      objectFit: "contain",
                      transform: tileImageTransform(positioning.scale, positioning.offsetX + 50, positioning.offsetY + 50, r, c, positioning.tileRows, positioning.tileCols),
                      transformOrigin: "center center",
                      pointerEvents: "none",
                    }}
                  />
