...
export default function App() {
  ...
  async function runAdvancedSearch() {
    ...
    try {
      ...
      if (advSet.trim()) params.push(`set.id=eq:${encodeURIComponent(advSet.trim())}`);
      ...
      let results = await searchCardsMultiLang(params.join("&"));

      // Fallback: if a set filter returned nothing (can happen for very new
      // sets where the set-card link lags behind the individual card entries),
      // fetch the set's own card list directly and cross-check against it.
      if (results.length === 0 && advSet.trim()) {
        const setCards = await fetchSetCards(advSet.trim());
        let fallback = setCards;
        if (advName.trim()) {
          const q = advName.trim().toLowerCase();
          fallback = fallback.filter((c) => c.name && c.name.toLowerCase().includes(q));
        }
        if (advNumber.trim()) {
          const q = advNumber.trim().toLowerCase();
          fallback = fallback.filter((c) => String(c.localId).toLowerCase() === q);
        }
        results = fallback;
      }
