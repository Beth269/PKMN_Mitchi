...
export default function App() {
  ...
  async function fetchSetCards(setId) {
    const [enSet, deSet] = await Promise.all([
      fetchWithRetry(`https://api.tcgdex.net/v2/en/sets/${setId}`).then((r) => r.json()).catch(() => null),
      fetchWithRetry(`https://api.tcgdex.net/v2/de/sets/${setId}`).then((r) => r.json()).catch(() => null),
    ]);
  ...
  async function runAdvancedSearch() {
    ...
    try {
      let results;
      if (advSet.trim()) {
        // Filtering the card list by nested set fields isn't reliable on TCGdex,
        // so fetch the set directly (it embeds its own card list) instead.
        ...
        params.push("pagination:itemsPerPage=80");
        results = await searchCardsMultiLang(params.join("&"));
      }
      setSearchResults(results);
      if (!results || results.length === 0) setSearchError("Keine Karten gefunden.");
      saveLastSearch();
    } catch (e) {
      setSearchError("Suche fehlgeschlagen. Prüfe deine Verbindung.");
    } finally {
