...
function PageGrid({ page, pageIdxGlobal, selected, mergeMode, mergeAnchor, onSlotTap, onSlotDoubleTap, onToggleMark, onPositionEdit, forExport, gridRef, big, highlightSlot }) {
        ...
        return (
          <button
            ...
            style={{
              ...
              aspectRatio: `${2.5 * spanCols} / ${3.5 * spanRows}`,
              minHeight: 0,
              borderRadius: 8,
              border: isSelected
              ...
              <>
                <img
                  ...
                  style={{
                    ...
                    transform: `translate(${(slot.offsetX ?? 50) - 50}%, ${(slot.offsetY ?? 50) - 50}%) scale(${slot.scale ?? 1})`,
...
export default function App() {
  ...
  function handleMergeTap(pageIdxGlobal, sIdx) {
    ...
    if (slot && (slot.spanNext || slot.spanDown)) {
      setBinders((prev) => {
        ...
        // Reset the crop/zoom since the slot's aspect ratio changes back to a single slot.
        slots[sIdx] = { ...slots[sIdx], spanNext: false, spanDown: false, offsetX: 50, offsetY: 50, scale: 1 };
    ...
    setBinders((prev) => {
      ...
      // Reset the crop/zoom to a fresh centered default for the new, larger merged
      // shape - carrying over a crop tuned for a single slot just cuts the image oddly.
      slots[primary] = primaryCard
        ? { ...primaryCard, spanNext: direction === "horizontal", spanDown: direction === "vertical", offsetX: 50, offsetY: 50, scale: 1 }
        : null;
  ...
  function openPositionEditor(pageIdxGlobal, sIdx) {
    ...
    setPositioning({
      ...
      scale: slot.scale ?? 1,
      image: slot.image,
      spanNext: !!slot.spanNext,
      spanDown: !!slot.spanDown,
    });
  }
  ...
  function savePositioning() {
    ...
    const { pageIdxGlobal, slotIdx, offsetX, offsetY, scale } = positioning;
    setBinders((prev) => {
      const next = prev.map((b) => ({ ...b, pages: b.pages.map((p) => ({ ...p, slots: [...p.slots] })) }));
      const slot = next[binderIndex].pages[pageIdxGlobal].slots[slotIdx];
      // Store back in the same 0-100 convention (50 = centered) so old and new
      // data always use one consistent format for rendering.
      if (slot) next[binderIndex].pages[pageIdxGlobal].slots[slotIdx] = { ...slot, offsetX: offsetX + 50, offsetY: offsetY + 50, scale };
      return next;
    });
    setPositioning(null);
  }
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div ref={spreadLeftRef}>
              <PageGrid page={leftPage} pageIdxGlobal={pageIndex} selected={selected} mergeMode={mergeMode} mergeAnchor={mergeAnchor} onSlotTap={handleSlotTap} onSlotDoubleTap={handleSlotDoubleTap} onToggleMark={toggleMark} onPositionEdit={openPositionEditor} highlightSlot={highlightSlot} />
            </div>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            {rightPage ? (
              <>
                ...
                <PageGrid page={rightPage} pageIdxGlobal={pageIndex + 1} selected={selected} mergeMode={mergeMode} mergeAnchor={mergeAnchor} onSlotTap={handleSlotTap} onSlotDoubleTap={handleSlotDoubleTap} onToggleMark={toggleMark} onPositionEdit={openPositionEditor} highlightSlot={highlightSlot} />
              </>
            ) : (
...
function PositionEditor({ positioning, onMove, onScale, onSave, onCancel }) {
  ...
  const aspect = positioning.spanNext ? "5 / 3.5" : positioning.spanDown ? "2.5 / 7" : "2.5 / 3.5";
