...
export default function App() {
  ...
  async function fetchSetCards(setId) {
    const [enResult, deResult] = await Promise.allSettled([
      fetchWithRetry(`https://api.tcgdex.net/v2/en/sets/${setId}`).then((r) => r.json()),
      fetchWithRetry(`https://api.tcgdex.net/v2/de/sets/${setId}`).then((r) => r.json()),
    ]);
    const enSet = enResult.status === "fulfilled" ? enResult.value : null;
    const deSet = deResult.status === "fulfilled" ? deResult.value : null;
    if (!enSet && !deSet) {
      const reason = enResult.status === "rejected" ? enResult.reason : deResult.reason;
      throw new Error("Set-Abruf fehlgeschlagen (" + setId + "): " + (reason && reason.message ? reason.message : "unbekannter Fehler"));
    }
    const all = [...((enSet && enSet.cards) || []), ...((deSet && deSet.cards) || [])];
    const seen = new Set();
    const merged = [];
    for (const c of all) {
      if (!c || !c.id || seen.has(c.id)) continue;
  ...
  async function runAdvancedSearch() {
    if (!advName.trim() && !advSet.trim() && !advRarity.trim() && !advArtist.trim() && !advNumber.trim()) {
    ...
    try {
      ...
      if (advSet.trim()) {
        // Nested "set.id"/"set.name" filters on the general /cards endpoint
        // are not reliable on TCGdex - use the dedicated set endpoint instead,
        // which embeds its own confirmed-working card list.
        results = await fetchSetCards(advSet.trim());
        if (advName.trim()) {
          const q = advName.trim().toLowerCase();
          results = results.filter((c) => c.name && c.name.toLowerCase().includes(q));
        }
        if (advNumber.trim()) {
          const q = advNumber.trim().toLowerCase();
          results = results.filter((c) => String(c.localId).toLowerCase() === q);
        }
        if ((advRarity.trim() || advArtist.trim()) && results.length > 0) {
          if (results.length > 60) {
            setSearchError("Zu viele Karten in diesem Set für die Kombination mit Seltenheit/Künstler — bitte zusätzlich einen Namen oder eine Nummer angeben.");
            results = [];
          } else {
            const fulls = await Promise.all(results.map((c) => fetchFullCard(c.id).catch(() => null)));
            results = results.filter((c, i) => {
              const full = fulls[i];
              if (!full) return false;
              if (advRarity.trim() && full.rarity !== advRarity.trim()) return false;
              if (advArtist.trim() && !(full.illustrator || "").toLowerCase().includes(advArtist.trim().toLowerCase())) return false;
              return true;
            });
          }
        }
        if (results.length === 0) {
          setSearchError("Für dieses Set sind bei TCGdex noch keine Karten verknüpft (kommt bei sehr neuen Sets vor). Versuch es stattdessen mit der Namenssuche.");
        }
      } else {
        const params = [];
        ...
        results = await searchCardsMultiLang(params.join("&"));
        if (results.length === 0) setSearchError("Keine Karten gefunden.");
      }
