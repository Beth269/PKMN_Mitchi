...
export default function App() {
  ...
  async function runSearch(q) {
    ...
    try {
      ...
      const results = (await searchCardsMultiLang(params)).filter((c) => c.image);
  ...
  async function fetchByCategory(resource, value) {
    ...
    const enList = enResult.status === "fulfilled" ? enResult.value : null;
    const deList = deResult.status === "fulfilled" ? deResult.value : null;
    if (enList === null && deList === null) {
      const reason = enResult.reason;
      throw new Error(`${resource}/${value} — ${reason && reason.message ? reason.message : "unbekannter Fehler"}`);
    }
    const combined = [...((Array.isArray(enList) && enList) || []), ...((Array.isArray(deList) && deList) || [])];
    if (combined.length === 0 && (enList !== null || deList !== null)) {
      // Requests succeeded but returned something unexpected (not a plain array) - surface that.
      const sample = enList !== null ? enList : deList;
      throw new Error(`${resource}/${value} — Antwort war kein erwartetes Array (${JSON.stringify(sample).slice(0, 120)})`);
    }
    return combined;
  }
  ...
  async function runAdvancedSearch() {
    ...
    try {
      ...
      const results = intersectById(resultSets).filter((c) => c.image);
      if (requestId !== searchRequestIdRef.current) return; // a newer search already started
      setSearchResults(results);
      if (results.length === 0) setSearchError("Keine Karten gefunden.");
      saveLastSearch();
