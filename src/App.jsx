...
async function toDataUrlCached(url) {
...
async function fetchWithRetry(url, retries = 2, delayMs = 700) {  let lastErr;
...
async function fetchAllSets() {
  const resp = await fetchWithRetry("https://api.pokemontcg.io/v2/sets?orderBy=-releaseDate");
  const data = await resp.json();
  return (data.data || []).map((s) => ({ id: s.id, name: s.name, series: s.series }));
}

async function fetchAllRarities() {
  const resp = await fetchWithRetry("https://api.pokemontcg.io/v2/rarities");
  const data = await resp.json();
  return data.data || [];
}
...
function extractPrice(card) {
  ...
  if (tp) {
    ...
    if (variant && typeof variant.market === "number") {
      return { amount: variant.market, currency: "USD" };
    }
  }
  const cmAvg = card.cardmarket && card.cardmarket.prices && card.cardmarket.prices.averageSellPrice;
  if (typeof cmAvg === "number") {
    return { amount: cmAvg, currency: "EUR" };
  }
  return null;
}
...
function translateName(input, germanMap) {  const lower = input.trim().toLowerCase();
...
export default function App() {
  ...
  async function refreshCardPrice(pageIdxGlobal, sIdx, cardId) {
    ...
    try {
      const resp = await fetchWithRetry(`https://api.pokemontcg.io/v2/cards/${cardId}`);
      const data = await resp.json();
  ...
  function resetSearchState() {
    ...
    try {
      ...
      if (res && res.value) {
        ...
        setAdvSet(s.advSet || ""); setAdvRarity(s.advRarity || "");
  ...
  function saveLastSearch() {
    localCache.set("last-search", JSON.stringify({ mode: searchMode, query: searchQuery, advName, advSet, advRarity, advArtist, advNumber }));
  }
  ...
  async function runAdvancedSearch() {
    ...
    if (advRarity.trim()) clauses.push(`rarity:"${advRarity.trim()}"`);
...
function SearchModal(props) {
  const {
    ...
    searchQuery, setSearchQuery, advName, setAdvName, advSet, setAdvSet, advRarity, setAdvRarity, raritiesList, raritiesLoading,
