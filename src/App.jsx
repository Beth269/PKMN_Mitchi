...
function fileToCompressedDataUrl(file, maxDim = 1400, quality = 0.9) {
...
async function fetchAllSets() {
  const resp = await fetchWithRetry("https://api.tcgdex.net/v2/en/sets?sort:field=releaseDate&sort:order=DESC");
...
// Older saves used gridColumn/gridRow spanning (spanNext/spanDown) for merged
// slots. That only supports rectangles, not L-shapes, so it's replaced by a
// per-cell "sprite tile" system (groupId + tileRow/tileCol/tileRows/tileCols).
// This converts any old-format pages to the new format on load.
function migratePageSlots(page) {
  const cols = page.cols;
  const slots = [...(page.slots || [])];
  for (let i = 0; i < slots.length; i++) {
    const s = slots[i];
    if (s && (s.spanNext || s.spanDown)) {
      const groupId = uid();
      const secondaryIdx = s.spanNext ? i + 1 : i + cols;
      const tileRows = s.spanDown ? 2 : 1;
      const tileCols = s.spanNext ? 2 : 1;
      const base = { ...s };
      delete base.spanNext;
      delete base.spanDown;
      delete base.placeholder;
      slots[i] = { ...base, groupId, tileRow: 0, tileCol: 0, tileRows, tileCols };
      if (secondaryIdx < slots.length) {
        slots[secondaryIdx] = { ...base, groupId, tileRow: s.spanDown ? 1 : 0, tileCol: s.spanNext ? 1 : 0, tileRows, tileCols };
      }
    }
  }
  return slots;
...
function normalizeLoadedData(parsed) {
  ...
  return binders.map((b) => ({
    ...
    cover: b.cover || null,
    pages: (b.pages || []).map((p) => {
      const migrated = migratePageSlots(p);
      return {
        id: p.id || uid(),
        blank: !!p.blank,
        rows: p.rows,
        cols: p.cols,
        slots: migrated.map((s) => (s && s.type ? { marked: false, ...s } : s || null)),
      };
    }),
  }));
}
...
// Draws exactly the right slice of a shared "virtual big picture" for one
// tile of a connected group, using a real <canvas> instead of CSS transforms.
// Canvas math is plain pixel arithmetic - there's no transform-composition
// ambiguity left to get wrong, so this is the reliable replacement for the
// CSS translate/scale approach that kept drifting out of alignment.
const tileImageCache = new Map();
function loadTileImage(src) {
  if (tileImageCache.has(src)) return tileImageCache.get(src);
  ...
  tileImageCache.set(src, promise);
  return promise;
}

function TileCanvas({ src, tileRow, tileCol, tileRows, tileCols, scale, offsetX, offsetY, style }) {
  ...
  useEffect(() => {
    ...
    cached.then((img) => {
      ...
      const compositeW = tileCols * CANVAS_W;
      const compositeH = tileRows * CANVAS_H;
      const imgAspect = img.naturalWidth / img.naturalHeight;
      const boxAspect = compositeW / compositeH;
      let baseW, baseH;
      if (imgAspect > boxAspect) {
        baseW = compositeW;
        baseH = compositeW / imgAspect;
      } else {
        baseH = compositeH;
        baseW = compositeH * imgAspect;
      }
      const s = scale ?? 1;
      ...
      const panXPct = (offsetX ?? 50) - 50;
      const panYPct = (offsetY ?? 50) - 50;
      const drawX = cx - scaledW / 2 + (panXPct / 100) * compositeW - tileCol * CANVAS_W;
      const drawY = cy - scaledH / 2 + (panYPct / 100) * compositeH - tileRow * CANVAS_H;
  ...
  }, [src, tileRow, tileCol, tileRows, tileCols, scale, offsetX, offsetY]);

  return <canvas ref={canvasRef} style={{ width: "100%", height: "100%", display: "block", ...style }} />;
}

// ================= Page grid subcomponent =================
function PageGrid({ page, pageIdxGlobal, selected, mergeMode, mergeAnchor, onSlotTap, onSlotDoubleTap, onToggleMark, onPositionEdit, onDissolveGroup, forExport, gridRef, big, highlightSlot }) {
  ...
  return (
    ...
    >
      {page.slots.map((slot, sIdx) => {
        ...
        const inGroup = !!(slot && slot.groupId);
        const tileRows = (slot && slot.tileRows) || 1;
        const tileCols = (slot && slot.tileCols) || 1;
        const tileRow = (slot && slot.tileRow) || 0;
        const tileCol = (slot && slot.tileCol) || 0;
              ...
              <>
                <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
                  {slot.type === "custom" ? (
                    ...
                    <img src={slot.image} alt={slot.name} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                  )}
                </div>
                {!forExport && (
                  <span
                ...
                {!forExport && slot.type === "custom" && (
                  <span
                    role="button"
                    onClick={(e) => {
                      e.stopPropagation();
                      onPositionEdit(pageIdxGlobal, sIdx);
                    }}
                    style={{
                      position: "absolute",
                      top: 4,
                ...
                {!forExport && mergeMode && inGroup && (
                  <span
                    role="button"
                    onClick={(e) => {
                      e.stopPropagation();
                      onDissolveGroup(pageIdxGlobal, sIdx);
                    }}
                    style={{
                      position: "absolute",
                      bottom: 4,
...
export default function App() {
  ...
  const [positioning, setPositioning] = useState(null); // {pageIdxGlobal, slotIdx, offsetX, offsetY}
  ...
  function handleMergeTap(pageIdxGlobal, sIdx) {
    ...
    if (anchor && anchor.pageIdxGlobal === pageIdxGlobal && anchor.slotIdx === sIdx) {
      setAnchor(null);
      return;
    }
    if (!anchor || anchor.pageIdxGlobal !== pageIdxGlobal) {
      setAnchor({ pageIdxGlobal, slotIdx: sIdx });
      setMergeError("");
      return;
    }

    const anchorSlot = page.slots[anchor.slotIdx];
    const anchorGroupId = anchorSlot && anchorSlot.groupId;
    const anchorMemberIdxs = [];
    if (anchorGroupId) {
      page.slots.forEach((s, idx) => {
        if (s && s.groupId === anchorGroupId) anchorMemberIdxs.push(idx);
      });
    } else {
      anchorMemberIdxs.push(anchor.slotIdx);
    }

    const tappedGroupId = slot && slot.groupId;
    if (tappedGroupId && tappedGroupId === anchorGroupId) {
      setAnchor({ pageIdxGlobal, slotIdx: sIdx });
      return;
    }

    const { row: tr, col: tc } = idxToRC(sIdx);
    const isAdjacent = anchorMemberIdxs.some((idx) => {
      const { row, col } = idxToRC(idx);
      return (row === tr && Math.abs(col - tc) === 1) || (col === tc && Math.abs(row - tr) === 1);
    });

    if (!isAdjacent || tappedGroupId) {
      // Not touching the current group, or already belongs to a different one - just re-anchor here.
      setAnchor({ pageIdxGlobal, slotIdx: sIdx });
      setMergeError("");
      return;
    }

    const allIdxs = [...anchorMemberIdxs, sIdx];
    const positions = allIdxs.map(idxToRC);
    ...
    const sourceSlot = anchorMemberIdxs.map((i) => page.slots[i]).find((s) => s && s.image) || (slot && slot.image ? slot : null);
    if (!sourceSlot) {
      setMergeError("Füge ein Foto hinzu um Slots zu verbinden.");
      setAnchor(null);
      return;
    }

    setMergeError("");
    const groupId = anchorGroupId || uid();
    setBinders((prev) => {
      const next = prev.map((b) => ({ ...b, pages: b.pages.map((p) => ({ ...p, slots: [...p.slots] })) }));
    ...
    setAnchor({ pageIdxGlobal, slotIdx: sIdx }); // keep growing from here if the user taps another neighbor
  }

  function dissolveGroup(pageIdxGlobal, sIdx) {
    const page = currentBinder.pages[pageIdxGlobal];
    const slot = page.slots[sIdx];
    ...
    setBinders((prev) => {
      const next = prev.map((b) => ({ ...b, pages: b.pages.map((p) => ({ ...p, slots: [...p.slots] })) }));
      const slots = next[binderIndex].pages[pageIdxGlobal].slots;
      // Keep the image in only the top-left cell of the group; the rest become empty slots.
      let keptOne = false;
      slots.forEach((s, idx) => {
        if (!s || s.groupId !== groupId) return;
        const isPrimary = s.tileRow === 0 && s.tileCol === 0;
        if (isPrimary && !keptOne) {
          const { groupId: _g, tileRow: _tr, tileCol: _tc, tileRows: _trs, tileCols: _tcs, ...rest } = s;
          slots[idx] = { ...rest, offsetX: 50, offsetY: 50, scale: 1 };
          keptOne = true;
        } else {
          slots[idx] = null;
        }
  ...
  function openPositionEditor(pageIdxGlobal, sIdx) {
    const page = currentBinder.pages[pageIdxGlobal];
    const slot = page.slots[sIdx];
    if (!slot || slot.type !== "custom") return;
    // Older saved crops used a 0-100 "object-position" style value (50 = centered).
    // The editor now works with a -50..50 translate offset (0 = centered) - convert once on open.
    const rawX = slot.offsetX ?? 50;
    const rawY = slot.offsetY ?? 50;
    const groupId = slot.groupId || null;
    const members = groupId
      ? page.slots.filter((s) => s && s.groupId === groupId).map((s) => ({ tileRow: s.tileRow, tileCol: s.tileCol }))
      : [{ tileRow: 0, tileCol: 0 }];
    setPositioning({
      pageIdxGlobal,
      slotIdx: sIdx,
      groupId,
      tileRows: slot.tileRows || 1,
      tileCols: slot.tileCols || 1,
      members,
      offsetX: rawX - 50,
      offsetY: rawY - 50,
      scale: slot.scale ?? 1,
  ...
  function updatePositioningOffset(dx, dy) {
    setPositioning((prev) => {
      if (!prev) return prev;
      // More zoom needs more room to pan, otherwise the edges become unreachable.
      const maxOffset = 50 * Math.max(1, prev.scale);
      return {
        ...prev,
        offsetX: Math.max(-maxOffset, Math.min(maxOffset, prev.offsetX + dx)),
        offsetY: Math.max(-maxOffset, Math.min(maxOffset, prev.offsetY + dy)),
      };
    });
  }

  function setPositioningScale(scale) {
    setPositioning((prev) => {
      if (!prev) return prev;
      const newScale = Math.max(0.3, Math.min(3, scale));
      const maxOffset = 50 * Math.max(1, newScale);
      return {
        ...prev,
        scale: newScale,
        offsetX: Math.max(-maxOffset, Math.min(maxOffset, prev.offsetX)),
        offsetY: Math.max(-maxOffset, Math.min(maxOffset, prev.offsetY)),
      };
    });
  }
  ...
  function savePositioning() {
    ...
    const { pageIdxGlobal, slotIdx, offsetX, offsetY, scale, groupId } = positioning;
    setBinders((prev) => {
      const next = prev.map((b) => ({ ...b, pages: b.pages.map((p) => ({ ...p, slots: [...p.slots] })) }));
      const slots = next[binderIndex].pages[pageIdxGlobal].slots;
      // Store back in the same 0-100 convention (50 = centered) so old and new
      // data always use one consistent format for rendering.
      const newOffsetX = offsetX + 50;
      const newOffsetY = offsetY + 50;
      if (groupId) {
        // Pan/zoom is shared across every cell in the group, not just this one.
        slots.forEach((s, i) => {
          if (s && s.groupId === groupId) slots[i] = { ...s, offsetX: newOffsetX, offsetY: newOffsetY, scale };
        });
      } else if (slots[slotIdx]) {
        slots[slotIdx] = { ...slots[slotIdx], offsetX: newOffsetX, offsetY: newOffsetY, scale };
      }
      return next;
    });
    setPositioning(null);
  ...
  function resetSearchState() {
    searchRequestIdRef.current++; // invalidate any still-pending search from a previous open
    setSearchResults([]);
    setSearchError("");
    setSelectedResults({});
  ...
  async function runSearch(q) {
    ...
    try {
      ...
      let results = (await searchCardsMultiLang(params)).filter((c) => c.image);
      results = await sortByReleaseDate(results);
      if (requestId !== searchRequestIdRef.current) return; // a newer search already started
  ...
  // Rarity and illustrator are "browse by category" resources on TCGdex, not
  // filterable query params on /cards - they need their own dedicated endpoint
  // (e.g. /v2/en/rarities/Illustration%20Rare), which returns the matching cards directly.
  async function fetchByCategory(resource, value) {
    const enUrl = `https://api.tcgdex.net/v2/en/${resource}/${encodeURIComponent(value)}`;
    const deUrl = `https://api.tcgdex.net/v2/de/${resource}/${encodeURIComponent(value)}`;
    const [enResult, deResult] = await Promise.allSettled([
      fetchWithRetry(enUrl).then((r) => r.json()),
      fetchWithRetry(deUrl).then((r) => r.json()),
    ]);
    const enBody = enResult.status === "fulfilled" ? enResult.value : null;
    const deBody = deResult.status === "fulfilled" ? deResult.value : null;
    if (enBody === null && deBody === null) {
      const reason = enResult.reason;
      throw new Error(`${resource}/${value} — ${reason && reason.message ? reason.message : "unbekannter Fehler"}`);
    }
    // The response is an object like { name: "...", cards: [...] }, not a bare array.
    const enList = enBody && Array.isArray(enBody.cards) ? enBody.cards : [];
    const deList = deBody && Array.isArray(deBody.cards) ? deBody.cards : [];
    const seen = new Set();
    const merged = [];
    for (const c of [...enList, ...deList]) {
      if (!c || !c.id || seen.has(c.id)) continue;
      seen.add(c.id);
      merged.push(c);
    }
  ...
  function intersectById(lists) {
    ...
    return lists.reduce((acc, list) => {
      ...
      return acc.filter((c) => ids.has(c.id));
    });
  }

  // Card results don't carry a release date - derive each card's set id from
  // its own id/localId, then look up that set's releaseDate (cached locally
  // so repeat searches don't refetch the same sets over and over).
  const setReleaseDateCacheRef = useRef({});

  async function sortByReleaseDate(cards) {
    const cache = setReleaseDateCacheRef.current;
    const setIds = new Set();
    for (const c of cards) {
      if (!c.id || !c.localId) continue;
      const setId = c.id.slice(0, c.id.length - String(c.localId).length - 1);
      if (setId && !(setId in cache)) setIds.add(setId);
    }
    const missing = Array.from(setIds);
    const batchSize = 15;
    for (let i = 0; i < missing.length; i += batchSize) {
      const batch = missing.slice(i, i + batchSize);
      const results = await Promise.allSettled(
        batch.map((id) => fetchWithRetry(`https://api.tcgdex.net/v2/en/sets/${encodeURIComponent(id)}`).then((r) => r.json()))
      );
      batch.forEach((id, j) => {
        const r = results[j];
        cache[id] = r.status === "fulfilled" && r.value && r.value.releaseDate ? r.value.releaseDate : "";
      });
    }
    return [...cards].sort((a, b) => {
      const setA = a.id.slice(0, a.id.length - String(a.localId).length - 1);
      const setB = b.id.slice(0, b.id.length - String(b.localId).length - 1);
      const dateA = cache[setA] || "";
      const dateB = cache[setB] || "";
  ...
  async function runAdvancedSearch() {
    if (!advName.trim() && !advRarity.trim() && !advArtist.trim() && !advNumber.trim() && !advSet.trim()) {
    ...
    try {
      ...
      if (advRarity.trim()) {
        resultSets.push(await fetchByCategory("rarities", advRarity.trim()));
      }
      if (advArtist.trim()) {
        resultSets.push(await fetchByCategory("illustrators", advArtist.trim()));
      }
      if (advName.trim() || advNumber.trim()) {
        const params = [];
        ...
        params.push("pagination:itemsPerPage=200");
        resultSets.push(await searchCardsMultiLang(params.join("&")));
      }

      let results;
      if (resultSets.length > 0) {
        // Reliable path: filter the already-fetched results by set, using each
        // card's own id/localId to derive its set - no separate (unreliable) set query needed.
        results = intersectById(resultSets);
        if (advSet.trim()) {
          results = results.filter((c) => {
            if (!c.id || !c.localId) return false;
            const setId = c.id.slice(0, c.id.length - String(c.localId).length - 1);
            return setId === advSet.trim();
          });
        }
      } else {
        // Only "Set" was filled in - fall back to asking that set directly for
        // its card list. Works reliably for established sets; very new sets
        // (days/weeks old) may not have this link populated yet on TCGdex.
        const [enResult, deResult] = await Promise.allSettled([
          fetchWithRetry(`https://api.tcgdex.net/v2/en/sets/${encodeURIComponent(advSet.trim())}`).then((r) => r.json()),
          fetchWithRetry(`https://api.tcgdex.net/v2/de/sets/${encodeURIComponent(advSet.trim())}`).then((r) => r.json()),
        ]);
        const enCards = enResult.status === "fulfilled" && Array.isArray(enResult.value?.cards) ? enResult.value.cards : [];
        const deCards = deResult.status === "fulfilled" && Array.isArray(deResult.value?.cards) ? deResult.value.cards : [];
        const seen = new Set();
        results = [];
        for (const c of [...enCards, ...deCards]) {
          if (!c || !c.id || seen.has(c.id)) continue;
          seen.add(c.id);
          results.push(c);
        }
      ...
      results = results.filter((c) => c.image);
      results = await sortByReleaseDate(results);
      if (requestId !== searchRequestIdRef.current) return; // a newer search already started
      setSearchResults(results);
      if (results.length === 0) {
        setSearchError(
          resultSets.length === 0 && advSet.trim()
            ? "Für dieses Set sind bei TCGdex noch keine Karten verknüpft (kommt bei sehr neuen Sets vor). Versuch es mit Name/Seltenheit/Künstler kombiniert."
  ...
  async function handleFileUpload(e) {
    ...
    try {
      ...
      applyToTarget([{ type: "custom", image: dataUrl, name: file.name.replace(/\.[^.]+$/, ""), marked: false }]);
    } catch (e) {
  ...
  function getMissingCards() {
    ...
    currentBinder.pages.forEach((page, pIdx) => {
      ...
      page.slots.forEach((slot, sIdx) => {
        if (slot && slot.image && !slot.marked) {
      ...
      <div style={{ background: COLORS.bg, minHeight: "100vh", color: COLORS.text, fontFamily: "'Segoe UI', system-ui, sans-serif", maxWidth: 560, margin: "0 auto" }}>
        ...
        {mergeMode && !zPage.blank && (
          <div style={{ background: COLORS.markGreen, color: "#06210C", padding: "8px 16px", fontSize: 13 }}>
            Slot antippen, dann weitere angrenzende Slots antippen zum Verbinden — auch mehrere und um die Ecke. Zum Trennen auf ✂ am Slot tippen.
          </div>
        )}
        ...
        <main style={{ padding: 6 }}>
          <PageGrid
            ...
            onPositionEdit={openPositionEditor}
        ...
        {positioning && (
          <PositionEditor
            ...
            onMove={updatePositioningOffset}
            onScale={setPositioningScale}
  ...
  return (
    <div style={{ background: COLORS.bg, minHeight: "100vh", color: COLORS.text, fontFamily: "'Segoe UI', system-ui, -apple-system, sans-serif", display: "flex", flexDirection: "column", maxWidth: 560, margin: "0 auto", position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div ref={spreadLeftRef}>
              <PageGrid page={leftPage} pageIdxGlobal={pageIndex} selected={selected} mergeMode={mergeMode} mergeAnchor={mergeAnchor} onSlotTap={handleSlotTap} onSlotDoubleTap={handleSlotDoubleTap} onToggleMark={toggleMark} onPositionEdit={openPositionEditor} onDissolveGroup={dissolveGroup} highlightSlot={highlightSlot} />
            </div>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            {rightPage ? (
              <>
                ...
                <PageGrid page={rightPage} pageIdxGlobal={pageIndex + 1} selected={selected} mergeMode={mergeMode} mergeAnchor={mergeAnchor} onSlotTap={handleSlotTap} onSlotDoubleTap={handleSlotDoubleTap} onToggleMark={toggleMark} onPositionEdit={openPositionEditor} onDissolveGroup={dissolveGroup} highlightSlot={highlightSlot} />
              </>
            ) : (
      ...
      {sheet === "reorderPages" && (
        <Sheet onClose={() => { setSheet(null); setDragIdx(null); }} title="Seiten anordnen" big>
          <p style={{ color: COLORS.textDim, fontSize: 13, marginBottom: 12 }}>
            {dragIdx === null
              ? "Tippe eine Seite an, um sie auszuwählen."
              : "Jetzt die Zielposition antippen, um die Seite dorthin zu verschieben (oder nochmal dieselbe Seite antippen zum Abbrechen)."}
          </p>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    padding: "10px 8px",
                    borderRadius: 8,
                    border: `2px solid ${isPicked ? COLORS.gold : COLORS.cardBorder}`,
                    background: isPicked ? COLORS.bgElevated : COLORS.card,
                    cursor: "pointer",
                  }}
                >
                  <div
                  ...
                  <p style={{ margin: 0, fontSize: 12, fontWeight: 600 }}>Seite {idx + 1}{isPicked ? " ✓" : ""}</p>
...
// ================= Reposition an uploaded image inside its slot =================
function PositionEditor({ positioning, onMove, onScale, onSave, onCancel }) {
  ...
  function handleTouchStart(e) {
    if (e.touches.length === 2) {
      const [a, b] = e.touches;
      pinchRef.current = { dist: Math.hypot(a.clientX - b.clientX, a.clientY - b.clientY), scale: positioning.scale };
  ...
  function handleTouchMove(e) {
    if (e.touches.length === 2 && pinchRef.current) {
      ...
      onScale(pinchRef.current.scale * factor);
    }
  }
  ...
  function startDrag(e) {
    if (pinchRef.current) return; // a two-finger pinch is in progress
  ...
  function moveDrag(e) {
    ...
    const dxPct = ((point.clientX - dragRef.current.x) / rect.width) * 100;
    const dyPct = ((point.clientY - dragRef.current.y) / rect.height) * 100;
    dragRef.current = { x: point.clientX, y: point.clientY };
    onMove(dxPct, dyPct);
  }
  function endDrag() {
  ...
  const aspect = `${2.5 * positioning.tileCols} / ${3.5 * positioning.tileRows}`;
  const memberSet = new Set(positioning.members.map((m) => `${m.tileRow},${m.tileCol}`));

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 45, padding: 24 }}>
      <p style={{ color: COLORS.text, fontSize: 13, marginBottom: 14, textAlign: "center" }}>
        Bild ziehen, um den Ausschnitt festzulegen — mit zwei Fingern zoomen
      </p>
      <div
        ref={boxRef}
        ...
        style={{
          ...
          display: "grid",
          gridTemplateColumns: `repeat(${positioning.tileCols}, 1fr)`,
          gridTemplateRows: `repeat(${positioning.tileRows}, 1fr)`,
          gap: 2,
          overflow: "hidden",
        }}
      >
        {Array.from({ length: positioning.tileRows }).map((_, r) =>
          Array.from({ length: positioning.tileCols }).map((_, c) => {
            const isMember = memberSet.has(`${r},${c}`);
            return (
              <div key={`${r}-${c}`} style={{ position: "relative", overflow: "hidden", background: isMember ? COLORS.card : "transparent" }}>
                {isMember && (
                  <TileCanvas
                    src={positioning.image}
                    tileRow={r}
                    tileCol={c}
                    tileRows={positioning.tileRows}
                    tileCols={positioning.tileCols}
                    scale={positioning.scale}
                    offsetX={positioning.offsetX + 50}
                    offsetY={positioning.offsetY + 50}
      ...
      <div style={{ width: "100%", maxWidth: 320, marginTop: 16 }}>
        <p style={{ color: COLORS.textDim, fontSize: 12, marginBottom: 6 }}>Zoom: {Math.round(positioning.scale * 100)}%</p>
        <input
          type="range"
          min="30"
          max="300"
          value={Math.round(positioning.scale * 100)}
          onChange={(e) => onScale(Number(e.target.value) / 100)}
          style={{ width: "100%" }}
        />
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 44px)", gridTemplateRows: "repeat(3, 44px)", gap: 4, marginTop: 14 }}>
        <span />
        <button onClick={() => onMove(0, -8)} style={arrowBtnStyle}>▲</button>
        <span />
        <button onClick={() => onMove(-8, 0)} style={arrowBtnStyle}>◀</button>
        <button onClick={() => onMove(0, 0)} style={{ ...arrowBtnStyle, opacity: 0.3 }}>✥</button>
        <button onClick={() => onMove(8, 0)} style={arrowBtnStyle}>▶</button>
        <span />
        <button onClick={() => onMove(0, 8)} style={arrowBtnStyle}>▼</button>
        <span />
      </div>
      <p style={{ color: COLORS.textDim, fontSize: 11, marginTop: 8 }}>Falls Ziehen nicht zuverlässig klappt: Pfeile antippen.</p>

      <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
        <button onClick={onCancel} style={secondaryBtnStyle}>Abbrechen</button>
        <button onClick={onSave} style={primaryBtnStyle}>Fertig</button>
      </div>
    </div>
  );
}
...
function SearchModal(props) {
  const {
    ...
    advArtist, setAdvArtist, advNumber, setAdvNumber, advSet, setAdvSet, setsList, setsLoading,
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {filtersExpanded && (
              <div>
                ...
                <input value={advArtist} onChange={(e) => setAdvArtist(e.target.value)} placeholder="z. B. Ken Sugimori" style={{ ...inputStyle, width: "100%", boxSizing: "border-box", marginBottom: 4 }} />
                <p style={{ fontSize: 10, color: COLORS.textDim, margin: "0 0 8px" }}>Bitte möglichst genauen/vollständigen Namen eingeben.</p>

                <label style={labelStyle}>Kartennummer</label>
                <input value={advNumber} onChange={(e) => setAdvNumber(e.target.value)} placeholder="z. B. 58" style={{ ...inputStyle, width: "100%", boxSizing: "border-box", marginBottom: 8 }} />

                <label style={labelStyle}>Set</label>
                <select
                  value={advSet}
                  onChange={(e) => setAdvSet(e.target.value)}
                  style={{ ...inputStyle, width: "100%", boxSizing: "border-box", marginBottom: 4, appearance: "auto" }}
                >
                ...
                <p style={{ fontSize: 10, color: COLORS.textDim, margin: "0 0 10px" }}>
                  Am zuverlässigsten in Kombination mit Name, Seltenheit oder Künstler.
                </p>
              </div>
            )}
        ...
        {searchResults.length > 0 && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
              {searchResults.filter((c) => c.image).map((c) => {
