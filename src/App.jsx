    ...
    await Promise.all(
      batch.map(async (s) => {
        try {
          ...
          if (deName) map[deName.name.toLowerCase()] = { en: enName ? enName.name : s.name, dex: d.id };
        } catch (e) {
          /* skip */
        } finally {
          completed += 1;
...
function translateName(input, germanMap) {
  const lower = input.trim().toLowerCase();
  if (germanMap && germanMap[lower]) return germanMap[lower].en;
  return input.trim();
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      else loadGermanMap(); // auto-build once in the background, no click needed
    } catch (e) {
      loadGermanMap();
    }
  ...
  function handleSlotDoubleTap(pageIdxGlobal, sIdx) {
    ...
    if (slot && !slot.placeholder) setEnlarge({ image: slot.image, name: slot.name, set: slot.set, number: slot.number });
  }
  ...
  function saveLastSearch() {
    localCache.set("last-search", JSON.stringify({ mode: searchMode, query: searchQuery, advName, advSet, advArtist, advNumber }));
  }
  ...
  async function runSearch(q) {
    ...
    setSearchError("");
    try {
      const englishName = translateName(q, germanMap);
      const url = `https://api.pokemontcg.io/v2/cards?q=name:${encodeURIComponent(englishName)}*&pageSize=250&orderBy=-set.releaseDate`;
  ...
  async function runAdvancedSearch() {
    const clauses = [];
    if (advName.trim()) clauses.push(`name:"${translateName(advName, germanMap)}*"`);
    ...
    try {
      ...
      const url = `https://api.pokemontcg.io/v2/cards?q=${encodeURIComponent(q)}&pageSize=250&orderBy=-set.releaseDate`;
      const resp = await fetch(url);
      if (!resp.ok) throw new Error("API antwortet nicht");
      const data = await resp.json();
      setSearchResults(data.data || []);
  ...
  function handlePageDragStart(idx, clientY) {
    setDragIdx(idx);
    dragIdxRef.current = idx;
    dragStartYRef.current = clientY;
  }

  function handlePageDragMove(clientY) {
    if (dragIdxRef.current === null) return;
    const delta = clientY - dragStartYRef.current;
    const steps = Math.round(delta / PAGE_ROW_HEIGHT);
    if (steps === 0) return;
    const from = dragIdxRef.current;
    const to = Math.max(0, Math.min(currentBinder.pages.length - 1, from + steps));
    if (to !== from) {
      movePage(from, to);
      ...
      <div style={{ background: COLORS.bg, minHeight: "100vh", color: COLORS.text, fontFamily: "'Segoe UI', system-ui, sans-serif", maxWidth: 560, margin: "0 auto" }}>
        ...
        {modal && (
          <SearchModal
            ...
            germanMap={germanMap}
            germanMapLoading={germanMapLoading}
            germanMapProgress={germanMapProgress}
            loadGermanMap={loadGermanMap}
            runSearch={runSearch}
    ...
    <div style={{ background: COLORS.bg, minHeight: "100vh", color: COLORS.text, fontFamily: "'Segoe UI', system-ui, -apple-system, sans-serif", display: "flex", flexDirection: "column", maxWidth: 560, margin: "0 auto", position: "relative" }}>
      <header style={{ padding: "18px 16px 10px", borderBottom: `1px solid ${COLORS.cardBorder}`, background: COLORS.bgElevated, position: "sticky", top: 0, zIndex: 5 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
          ...
          <button onClick={exportPagePDF} disabled={exporting} style={secondaryBtnStyle}>Seite als PDF</button>
          <button onClick={exportBinderPDF} disabled={exporting} style={secondaryBtnStyle}>Ordner als PDF</button>
        </div>
      </header>
      ...
      <main style={{ padding: 12, flex: 1 }}>
        ...
        <div style={{ marginTop: 20, display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <button onClick={() => openRename(binderIndex)} style={secondaryBtnStyle}>Binder umbenennen</button>
        </div>
      </main>
      ...
      {sheet === "reorderPages" && (
        <Sheet onClose={() => { setSheet(null); handlePageDragEnd(); }} title="Seiten anordnen" big>
          <p style={{ color: COLORS.textDim, fontSize: 13, marginBottom: 12 }}>
            Am Griff ⠿ halten und nach oben oder unten ziehen, um die Reihenfolge zu ändern.
          </p>
          <div
            onPointerMove={(e) => handlePageDragMove(e.clientY)}
                ...
                >
                  <span
                    onPointerDown={(e) => {
                      ...
                      handlePageDragStart(idx, e.clientY);
                    }}
                    style={{ cursor: "grab", fontSize: 20, color: COLORS.textDim, padding: "4px 6px", touchAction: "none" }}
                  >
                    ⠿
                  </span>
                  <div
                    style={{
                      ...
                      height: 56,
                      borderRadius: 4,
                      background: p.blank ? "#000" : COLORS.slotEmpty,
                      overflow: "hidden",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                  ...
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 600 }}>Seite {idx + 1}</p>
                    <p style={{ margin: 0, fontSize: 11, color: COLORS.textDim }}>
                      {p.blank ? "Leere Seite" : `${p.rows}×${p.cols} Slots`}
                    </p>
                  </div>
                </div>
              );
      ...
      {sheet === "newpage" && (
        <Sheet onClose={() => setSheet(null)} title="Neue Seite">
          ...
          <p style={{ color: COLORS.textDim, fontSize: 13, marginBottom: 10 }}>Oder als erste Seite (Titelseite) einfügen:</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 18 }}>
            {PAGE_SIZE_OPTIONS.map((opt) => (
              <button key={"start-" + opt.label} onClick={() => addPageAtStart(opt.rows, opt.cols)} style={optionBtnStyle}>
                {opt.label} als erste Seite
              </button>
            ))}
          </div>
...
function SearchModal(props) {
  const {
    ...
    searchQuery, setSearchQuery, advName, setAdvName, advSet, setAdvSet,
    advArtist, setAdvArtist, advNumber, setAdvNumber, setsList, setsLoading,
    germanMap, germanMapLoading, germanMapProgress, loadGermanMap,
      ...
      <div style={{ padding: 16, overflowY: "auto", flex: 1 }}>
        ...
        {searchMode === "simple" ? (
          <div style={{ marginBottom: 12 }}>
            ...
            {!germanMap && (
              <button onClick={loadGermanMap} disabled={germanMapLoading} style={linkBtnStyle}>
                {germanMapLoading ? `Lade deutsche Namen … ${germanMapProgress?.done ?? 0}/${germanMapProgress?.total ?? 1025}` : "Deutsche Namen aktivieren (einmalig laden)"}
              </button>
            )}
            {germanMap && <p style={{ fontSize: 11, color: COLORS.textDim, margin: "4px 0 0" }}>Deutsche Namen aktiv ✓</p>}
          </div>
        ) : (
          <div style={{ marginBottom: 12 }}>
            ...
            {filtersExpanded && (
              <div>
                ...
                <input value={advSet} onChange={(e) => setAdvSet(e.target.value)} placeholder="z. B. Base Set, Obsidian Flames" list="set-options" style={{ ...inputStyle, width: "100%", boxSizing: "border-box", marginBottom: 8 }} />
                {setsList && (
                  <datalist id="set-options">
                    {setsList.map((s) => <option key={s.id} value={s.name} />)}
                  </datalist>
                )}
                ...
                return (
                  <div key={c.id} style={{ position: "relative" }}>
                    <button
                      ...
                      style={{ border: `1px solid ${isChecked ? COLORS.gold : COLORS.cardBorder}`, borderRadius: 6, padding: 0, overflow: "hidden", background: COLORS.card, cursor: "pointer", width: "100%", display: "block" }}
                      title={c.name}
                    >
