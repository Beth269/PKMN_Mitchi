...
export default function App() {
  ...
  async function fetchByCategory(resource, value) {
    const [enResult, deResult] = await Promise.allSettled([
      fetchWithRetry(`https://api.tcgdex.net/v2/en/${resource}/${encodeURIComponent(value)}`).then((r) => r.json()),
      fetchWithRetry(`https://api.tcgdex.net/v2/de/${resource}/${encodeURIComponent(value)}`).then((r) => r.json()),
    ]);
    const enList = enResult.status === "fulfilled" ? enResult.value : [];
    const deList = deResult.status === "fulfilled" ? deResult.value : [];
    return [...(enList || []), ...(deList || [])];
  }
